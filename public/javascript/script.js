...
function generatePipe() {
    ...
    let gapStart =
        Math.floor(Math.random() * (maxGapBottom - minGapTop + 1)) + minGapTop;
    ...
    for (let i = 0; i < pipes.length; i++) {
        ...
        if (gameStarted && !gameOver) {
            ...
            if (
                ...
                (bY < pipes[i].topHeight || bY + bird.height > pipes[i].bottomY)
            ) {
                gameOver = true;
            }
        }
...
function drawLeaderboard() {
    ...
    ctx.fillText("Press L to close", canvas.width / 2 - 50, canvas.height - 20);
}
